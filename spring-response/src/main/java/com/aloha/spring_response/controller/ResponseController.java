 ...
 * 💛@Controller와 @ResponseBody 를 합한 역할을 하는 어노테이션 
 * ✅@Controller              ➡ View 반환 
 * ✅@RestController          ➡ 응답데이터 (메시지[상태코드, 응답헤도, 응답본문(body)])를 반환 
 */
...
public class ResponseController {
    ...
    public ResponseEntity<byte[]> download() throws IOException {
        ...
        // 파일을 다운로드 받도록 응답
        HttpHeaders headers = new HttpHeaders();
        headers.setContentType(MediaType.APPLICATION_OCTET_STREAM);     // 헤더 세팅
        headers.setContentDispositionFormData("attachment", "sample.png");      // attachment를 하면 다운로드 하도록 해줌
        return new ResponseEntity<>(bytes, headers, HttpStatus.OK);
    }
}
